...
const Signup = () => {
  const { signup, signInWithGoogle, error, setError, getDashboardUrl } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { user, profile, isNewUser } = await signInWithGoogle(formData.role);
      ...
      <div className="relative w-full max-w-lg">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Role Selection - Required for both methods */}
            <div>
            ...
            <button
              ...
              className={`
                ...
                ${isLoading || !formData.role ? 'opacity-50 cursor-not-allowed' : ''}
              `}
            >
